...
#pgm = range(C_MAXLINES + 1)
#pgm = ["" for x in range(C_MAXLINES + 1)]
#pgm = [[] for k in range(C_MAXLINES + 1)]
#pgm = [0 for _ in range(C_MAXLINES + 1)]
pgm = [""] * (C_MAXLINES + 1) # code stored here
vars = [0] * (C_MAXLINES + 1) # variable store
...
def initlex2():
  ...
  thelin = ""
  if curline < len(pgm): thelin = pgm[curline]
  thech = " "
  nexttok()
